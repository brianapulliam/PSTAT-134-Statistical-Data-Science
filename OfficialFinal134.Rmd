...
author: "Leah Paredes, Briana Pulliam, Tucker Kupfer, Valeria Lopez"
...
AL_corr <- AL_complete %>% 
  select(-date) %>% 
  cor() %>% 
  corrplot(type = "lower", diag = F, method = "number", number.cex = 0.4, title = "Correlation Plot for Alabama Variables")

WA_corr <- WA_complete %>% 
  select(-date) %>% 
  cor() %>% 
  corrplot(type = "lower", diag = F, method = "number", number.cex = 0.4, title = "Correlation Plot for Washington Variables")
```

From the correlation plot we can see that the variables are all positively correlated with some variables being stronger correlated with others. We see that the variables which have the highest correlations `death` are `positive` (0.98), `hospitalized` (0.97) and `positiveCasesViral` (0.97), meaning as `death` increases so do the other variables. On the other hand, the variables which are the least correlated with `death` are `totalTestResultsIncrease` (0.16) and `hospitalizedIncrese` (0.15) meaning as `death` increased there did not seem to be much of a upward trend or downward trend pattern with those variables.

Now we want to take a look at the `death` variable over time for each of the states.
...
When looking at the graph for Alabama and then for Washington we see that they seem to be similar. Some of the similarities we see are the upward trend for the data, which is to be expected, although there is odd jump up and then down in total deaths at the beginning of the graphs which does not make sense and could be further investigated in the future. One aspect to note is the ranges of the y-axis in the graphs representing deaths. Although the graphs appear to be very similar and follow the same trend over time, the range for Alabama is 0 to a little over 10,000 whereas the range for Washington is 0 to a little over 5,000. 

Now in order to visualize the differences in the ranges of the states we will set the ranges of the graphs to be the same. 
...
From the graphs, the difference in the number of deaths is now more more apparent, showing that Washington had nearly half the amount of deaths when comparing to Alabama. 
# Methods 
## Model Fitting

We chose to use Generalized Additive Models (GAMs) with the Poisson family because the response variable, COVID-19 deaths, is count data and is best modeled using a distribution suitable for non-negative integer values. The Poisson family, coupled with a log link function, allows for modeling the relationship between predictors and the response variable while accounting for the exponential nature of count data.

GAMs are particularly well-suited for this analysis as they allow for flexible, nonlinear relationships between the predictors and the response. By applying thin-plate regression splines, the model can effectively capture complex trends in the data, such as temporal changes in deaths and nonlinear effects of predictors like positive cases or hospitalizations.
